...
public class DatabaseHandler extends SQLiteOpenHelper {
	...
	// Adding new TUTORIAL
	void addTutorial(Tutorial tuts) {
		SQLiteDatabase db = this.getWritableDatabase();

		ContentValues values = new ContentValues();
		values.put(KEY_TUTCAT, tuts.getCat());
		values.put(KEY_TUTQUES, tuts.getQues()); // Contact Name
	...
	// Getting single tuts
	Tutorial getTutorial(String cat) {
		SQLiteDatabase db = this.getReadableDatabase();

		Cursor cursor = db.query(TABLE_TUTORIAL, new String[] { KEY_TUTID,
				KEY_TUTCAT, KEY_TUTQUES }, KEY_TUTCAT + "=?",
				new String[] { String.valueOf(cat) }, null, null, null, null);
		if (cursor != null)
			cursor.moveToFirst();

		Tutorial v = new Tutorial(Integer.parseInt(cursor.getString(0)),
				cursor.getString(1), cursor.getString(2));
		// return contact
		return v;
	}
	...
	// Getting All Tuts
	public List<Tutorial> getAllTutorial(String cat) {
		List<Tutorial> quantsList = new ArrayList<Tutorial>();
		// Select All Query
		String selectQuery = "SELECT  * FROM " + TABLE_TUTORIAL + " where "
				+ KEY_TUTCAT + "=" + "'" + cat + "'";
		...
		if (cursor.moveToFirst()) {
			do {
				...
				Tutorial quants = new Tutorial();
				quants.setID(Integer.parseInt(cursor.getString(0)));
				quants.setCat(cursor.getString(1));
				quants.setQues(cursor.getString(2));
